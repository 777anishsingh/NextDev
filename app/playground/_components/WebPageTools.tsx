...
import { Code2, Download, Ghost, Monitor, SquareArrowOutUpRight, Tablet, TabletSmartphone } from "lucide-react"
...
function WebPageTools({ selectedScreenSize, setSelectedScreenSize, generatedCode }: any) {
    ...
    return (
        <div className="p-2 shadow mt-2 border rounded-xl w-full flex items-center justify-between">
            <div className="flex gap-2">
                <Button variant={'ghost'}
                    className={`${selectedScreenSize == 'web' ? 'border-2 border-primary bg-zinc-300' : null}`}
                    onClick={() => setSelectedScreenSize('web')}>
                    <Monitor />
                </Button>

                <Button variant={'ghost'}
                    className={`${selectedScreenSize == 'mobile' ? 'border-2 border-primary bg-zinc-300' : null}`}
